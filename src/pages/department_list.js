...
import {
  ...
  Department_configuration_update,
  DeleteDepartment_by_id,
...
const Department_list = () => {
  ...
  const pageAccessData = async () => {
    try {
      const resp = await get_Client_Page_Access(2);
      console.log(resp);
      if (resp.status === 200) {
  ...
  const handleConfigurationSave = async () => {
    ...
    try {
      const res = await Department_configuration_update(newData);
      console.log(res);
      if (res.status === 200) {
  ...
  const DeleteDepartment = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await DeleteDepartment_by_id(id);
          if (res) {
            if (res.status === 200) {
              fetchData();
              var message = "Department Record deleted Successfully";
  ...
  const ExportDepartment = async () => {
    try {
      ...
      console.log(res);
      if (res.status == 200) {
        var message = "Department Export Successfully";
        handleExceptionSuccessMessages(message);
        const url = res.data;
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          if (result.status === 200) {
            var message = "Department Record Import Successfully";
            handleExceptionSuccessMessages(message);
            fetchData();
            setIsOptionsDropdownOpen(false);
  ...
  const ViewImportFileFormat = async () => {
    try {
      const url =
        "http://hrmsapi.resolutesolutions.in.net/uploads/ClientExportExcelDemo/DepartmentDemoRpt.xlsx";
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "data.csv");
      document.body.appendChild(link);
  ...
  const skipAndContinueImport = async () => {
    try {
      ...
      if (res) {
        if (res.status === 200) {
          fetchData();
          var message = "Department Records Imported Successfully";
          handleExceptionSuccessMessages(message);
        } else if (res.status === 400) {
